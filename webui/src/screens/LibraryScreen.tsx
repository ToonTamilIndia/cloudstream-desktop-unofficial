...
export default function LibraryScreen() {
  ...
  const handleItemClick = (item: LibraryItem) => {
    navigate(
      `/details?url=${encodeURIComponent(item.url)}&api=${encodeURIComponent(item.apiName)}&name=${encodeURIComponent(item.name)}`,
    )
  }
      ...
      <Box sx={{ flex: 1, overflow: 'auto', px: 3, py: 1 }}>
        {items.length === 0 ? (
          <Box sx={{ textAlign: 'center', mt: 8 }}>
            ...
            <Button variant="contained" onClick={() => navigate('/')} sx={{ bgcolor: desktopTheme.accent, borderRadius: 2 }}>
              Browse Shows
            </Button>
          </Box>
        ) : (
                ...
                <Card
                  ...
                  sx={{
                    ...
                    '&:hover': {
                      ...
                      boxShadow: `0 4px 16px ${desktopTheme.accent}30`,
                    },
                    position: 'relative',
                    overflow: 'hidden',
                  }}
                >
                  <Box sx={{ position: 'relative', pt: '150%' }}>
                    {item.posterUrl ? (
                      ...
                      >
                        <Typography variant="h4" sx={{ color: desktopTheme.textMuted, fontWeight: 700 }}>
                          {item.name[0]}
                        </Typography>
                      </Box>
                    )}
